...
MQTT_BROKER_HOST = "192.168.1.6"  # Your local MQTT gateway IP
MQTT_BROKER_PORT = 1883
MQTT_GATEWAY_IP = "192.168.1.6"  # Your mqtt-gateway server IP
...
ENABLE_SEQUENCE_LOGGING = True  # Set to False to disable sequence logging
...
def generate_mqtt_credentials(mac_address: str) -> Dict[str, str]:
    ...
    # Format: GID_test@@@mac_address@@@uuid (for v2 protocol)
    client_id = f"{group_id}@@@{mac_address}@@@{client_uuid}"
    
    # Create user data and encode as base64 JSON (as expected by the gateway)
    user_data = {"ip": "192.168.1.100"}  # Client IP
    username = base64.b64encode(json.dumps(user_data).encode()).decode()
    
    # Generate password signature (HMAC-SHA256)
    signature_key = "test-signature-key-12345"  # Should match .env file
...
class TestClient:
    def __init__(self):
        ...
        print(f"MQTT Client ID: {self.mqtt_credentials['client_id']}")
        
        # The P2P topic will now be unique to this client's MAC address
        self.p2p_topic = f"devices/p2p/{self.device_mac_formatted}"
        self.device_server_topic = "device-server"  # Topic to send messages to server
    ...
    def on_mqtt_connect(self, client, userdata, flags, rc, properties=None):
        ...
        if rc == 0:
            logger.info(f"✅ MQTT Connected to gateway! Subscribing to P2P topic: {self.p2p_topic}")
    ...
    def on_mqtt_message(self, client, userdata, msg):
        ...
        try:
            ...
            # Handle hello response from gateway
            if payload.get("type") == "hello" and "udp" in payload:
                logger.info("🤝 Received hello response with UDP details")
            ...
            # Handle goodbye message
                ...
                logger.info("👋 Received goodbye from server")
    ...
    def send_hello_message(self):
        """Send hello message to the gateway."""
        hello_payload = {
            "type": "hello",
            "version": 3,
            "transport": "mqtt",
        ...
        logger.info("📤 Sending hello message to gateway...")
        self.mqtt_client.publish(self.device_server_topic, json.dumps(hello_payload))
    ...
    def retry_conversation(self):
        ...
        if self.session_active and not self.tts_active:
            ...
            if self.conversation_count < 3:  # Limit retries
                ...
                if self.mqtt_client:
                    self.mqtt_client.publish(self.device_server_topic, json.dumps(listen_payload))
            else:
    ...
    def setup_mqtt_connection(self) -> bool:
        """Setup MQTT connection to the gateway."""
        logger.info(f"▶️ STEP 1: Setting up MQTT connection to gateway at {MQTT_BROKER_HOST}:{MQTT_BROKER_PORT}")
        return True  # No OTA config needed for gateway connection

    def connect_mqtt(self) -> bool:
        """Connects to the MQTT Gateway."""
        logger.info("▶️ STEP 2: Connecting to MQTT Gateway...")
    ...
    def wait_for_session_details(self) -> bool:
        """Wait for session details from the gateway after hello."""
        logger.info("▶️ STEP 3: Waiting for session details from gateway...")
        try:
            ...
            if response.get("type") == "hello" and "udp" in response:
                ...
                ping_payload = f"ping:{self.session_id}".encode()
                encrypted_ping = self.encrypt_packet(ping_payload)
                server_udp_addr = (MQTT_GATEWAY_IP, udp_session_details['udp']['port'])
                
                logger.info(f"🔄 Sending UDP Ping to {server_udp_addr} with session ID {self.session_id}")
                if encrypted_ping:
            ...
            logger.error("❌ Timed out waiting for 'hello' response from gateway.")
            return False
    ...
    def listen_for_udp_audio(self):
        ...
        logger.info(f"🎧 Audio decoder initialized: {audio_params['sample_rate']}Hz, {audio_params['channels']}ch, {audio_params['frame_duration']}ms frames ({frame_size_samples} samples)")
        ...
        while not stop_threads.is_set() and self.session_active:
            try:
                ...
                if data and len(data) > 16:
                    ...
                    if self.total_packets_received <= 3:
                        logger.info(f"🔐 Decrypting: header={header.hex()}, key={aes_key.hex()}")
                        logger.info(f"🔐 Encrypted first 8 bytes: {encrypted[:8].hex()}")
                    ...
                    if self.total_packets_received <= 3:
                        logger.info(f"🔐 Decrypted first 8 bytes: {opus_payload[:8].hex()}")
                    
                    # Debug: Log first few bytes of opus payload
                    if self.total_packets_received <= 3:
                        logger.info(f"🔍 Header info: {header_info}")
                        logger.info(f"🔍 Encrypted length: {len(encrypted)}, Opus payload length: {len(opus_payload)}")
                        logger.info(f"🔍 First 16 bytes of opus: {opus_payload[:16].hex()}")
                    
                    # Validate opus payload length matches header
                    if header_info and header_info.get('payload_len', 0) != len(opus_payload):
                        logger.warning(f"⚠️ Payload length mismatch: header says {header_info.get('payload_len')}, actual {len(opus_payload)}")
                    ...
                    try:
                        ...
                        logger.error(f"❌ Opus decode error: {e}, payload length: {len(opus_payload)}")
                        if self.total_packets_received <= 3:
                            logger.error(f"🔍 Failed opus data: {opus_payload.hex()}")
    ...
    def trigger_conversation(self):
        ...
        listen_payload = {"type": "listen", "session_id": self.session_id, "state": "detect", "text": "hello baby"}
        self.mqtt_client.publish(self.device_server_topic, json.dumps(listen_payload))
        ...
        def monitor_spacebar():
            while not stop_threads.is_set() and self.session_active:
                if keyboard.is_pressed('space'):
                    ...
                    self.mqtt_client.publish(self.device_server_topic, json.dumps(abort_payload))
    ...
    def cleanup(self):
        ...
        if self.mqtt_client and self.session_id:
            goodbye_payload = { "type": "goodbye", "session_id": self.session_id }
            self.mqtt_client.publish(self.device_server_topic, json.dumps(goodbye_payload))
    ...
    def run_test(self):
        ...
        if not self.setup_mqtt_connection(): return
        if not self.connect_mqtt(): return
        time.sleep(2) # Give MQTT a moment to connect, subscribe, and send hello
        if not self.wait_for_session_details():
            self.cleanup()
            return
